...
//        }
    ...
    public function payments_rep() {
        ...
        $this->db->from('ts_order_batches b');
        $this->db->join('ts_orders o','o.order_id=b.order_id');
        $this->db->where('b.batch_date >= ', strtotime('2025-01-01'));
        $this->db->where('b.batch_date < ', strtotime('2026-01-01'));
        ...
        $file_name = $this->config->item('upload_path_preload').'payment_report_2025.csv';
        @unlink($file_name);
...
//         $this->db->select('*')->from('sb_items')->where('item_template', $normal_template)->order_by('item_number');
        $this->db->select('*')->from('sb_items')->where('brand','BT')->order_by('item_number');
        $items = $this->db->get()->result_array();
        $this->load->config('uploader');
...
//        }
        for ($j=1; $j<11; $j++) {
            array_push($titles,'Qty '.$j);
            array_push($titles,'Price '.$j);
            array_push($titles, 'Sale '.$j);
        }
...
//            }
            ...
            foreach ($promprices as $promprice) {
                if (empty($promprice['price']) && empty($promprice['sale_price'])) {
                } else {
...
//        $max_year = date('Y', $years['maxdate']);
        ...
        foreach ($brands as $brand) {
            ...
            for ($i=$min_year; $i<=$max_year; $i++) {
                ...
                $this->db->select('o.order_id, o.order_num, o.order_cog, o.profit, o.profit_perc, o.revenue, o.order_date as amount_date, a.amount_sum, a.printshop_total');
                ...
                if (count($results) > 0) {
                    if ($nsheet==0) {
                        ...
                        $spreadsheet->setActiveSheetIndex($nsheet);
                        $sheet = $spreadsheet->getActiveSheet();
                    ...
                    $sheet->setCellValue('D1', 'COG');
                    $sheet->setCellValue('E1', 'Profit');
                    $sheet->setCellValue('F1', 'Profit %');
                    $sheet->setCellValue('G1', 'Fixed COG');
                    $sheet->setCellValue('H1', 'Fixed Profit');
                    $sheet->setCellValue('I1', 'Fixed Profit %');
                    $order_keys = [];
                    $orders = [];
                    $numrow = 2;
                    foreach ($results as $result) {
                        if (!in_array($result['order_id'], $order_keys)) {
                            ...
                            $fixperc = round($fixprofit/$result['revenue']*100,1);
                            $sheet->setCellValue('A'.$numrow, $result['order_num']);
                            $sheet->setCellValue('B'.$numrow, date('m/d/Y', $result['amount_date']));
                            $sheet->setCellValue('C'.$numrow, $result['revenue']);
                            $sheet->setCellValue('D'.$numrow, $result['order_cog']);
                            $sheet->setCellValue('E'.$numrow, $result['profit']);
                            $sheet->setCellValue('F'.$numrow, $result['profit_perc']);
                            $sheet->setCellValue('G'.$numrow, $fixcog);
                            $sheet->setCellValue('H'.$numrow, $fixprofit);
                            $sheet->setCellValue('I'.$numrow, $fixperc);
                            $numrow++;
                        }
                    }
                    $nsheet++;
            ...
            // Save file
            $writer = new Xlsx($spreadsheet); // instantiate Xlsx
            $filename = $this->config->item('upload_path_preload').'change_cog_'.$brand.'.xlsx';
            @unlink($filename);
...
//        echo 'File '.$filenorm.' ready'.PHP_EOL;
        ...
        // Extra Prices
        $this->db->select('p.item_price_setup, p.item_sale_setup, p.item_price_print, p.item_sale_print');
        $this->db->from('sb_items i');
        ...
        $sheet->setCellValue('E1','Print Cost');
        $sheet->setCellValue('F1','Setup Cost');
        $sheet->setCellValue('G1', 'Qty/Prices');
        // $sheet->setCellValue('C1', 'Qty/Prices');
        $numrow = 2;
        foreach ($items as $item) {
            $sheet->setCellValue('A'.$numrow, $item['item_number']);
            ...
            if (!empty($item['item_sale_print'])) {
                $sheet->setCellValue('E'.$numrow, $item['item_sale_print']);
            } else {
                $sheet->setCellValue('E'.$numrow, $item['item_price_print']);
            }
            if (!empty($item['item_sale_setup'])) {
                $sheet->setCellValue('F'.$numrow, $item['item_sale_setup']);
            } else {
                $sheet->setCellValue('F'.$numrow, $item['item_price_setup']);
            }
            // Prices
            $prices = $this->db->select('item_qty, sale_price')->from('sb_promo_price')->where('item_id', $item['item_id'])->where('sale_price != ',0)->order_by('item_qty')->get()->result_array();
            $cellnum = 71; // 67;
