...
<?php foreach ($items as $row) { ?>
    <div class="items_table_line bord_b <?=($nrow%2==0 ? '' : 'items_line_gray')?>">
        ...
        <div class="items_content_each2 bord_l">
            <input type="text" class="orderitem_price input_text_right orderitem_input input_border_gray <?=$row['qtyinput_class']?>" title="<?=$row['qtyinput_title']?>" data-field="item_price" data-item="<?=$row['item_id']?>" data-orderitem="<?=$row['order_item_id']?>" value="<?=number_format($row['item_price'],2)?>" />
        </div>
