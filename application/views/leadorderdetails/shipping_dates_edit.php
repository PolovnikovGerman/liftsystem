...
<div class="datevalue">
    <?php if ($user_role=='masteradmin') { ?>
        <input type="text" class="input_border_gray rushpastvalue <?=isset($shipping['shipdate_class']) ? $shipping['shipdate_class'] : ''?>" readonly id="rushpast" value="<?=empty($shipping['shipdate']) ? '' : date('m/d/Y', $shipping['shipdate']) ?>"/>
    <?php } else { ?>
