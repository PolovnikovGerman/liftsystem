...
<?php foreach ($orders as $row) {?>
    <div class="rowdata <?=(($nrow%2)==0 ? 'whitedatarow' : 'greydatarow')?> <?=($row['order_rush']==0 ? '' : 'rushgeneralorder')?>" data-orderid="<?=$row['order_id']?>">
        ...
        <div class="art artstage <?=$row['art_class']?> <?=$row['art_title']?>" data-orderid="<?=$row['order_id']?>" data-messageview="<?=$row['lastmsg']?>" >
            <?=$row['art_cell']?>
        </div>
        <div class="art artstage <?=$row['redrawn_class']?> <?=$row['redrawn_title']?>" data-orderid="<?=$row['order_id']?>" data-messageview="<?=$row['lastmsg']?>" >
            <?=$row['redrawn_cell']?>
        </div>
        <div class="art artstage <?=$row['vectorized_class']?> <?=$row['vectorized_title']?>" data-orderid="<?=$row['order_id']?>" data-messageview="<?=$row['lastmsg']?>" >
            <?=$row['vectorized_cell']?>
        </div>
        <div class="art artstage <?=$row['proofed_class']?> <?=$row['proofed_title']?>" data-orderid="<?=$row['order_id']?>" data-messageview="<?=$row['lastmsg']?>">
            <?=$row['proofed_cell']?>
        </div>
