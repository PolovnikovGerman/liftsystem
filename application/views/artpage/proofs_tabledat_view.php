...
<?php foreach ($email_dat as $row) {?>
    <div class="proof_tabrow <?=($nrow%2==0 ? 'whitedatarow' : 'greydatarow')?> <?=$row['rowclass']?>" id="profrow<?=$row['email_id']?>">
        ...
        <div class="artdataarea" data-proofid="<?=$row['email_id']?>">
            <div class="proof_art artdata <?=$row['art_class']?>  <?=$row['art_title']?>">
<!--                data-messageview="--><?php //=$row['lastmsg']?><!--"                -->
                <?=$row['art_cell']?>
            </div>
            <div class="proof_redrawn artdata <?=$row['redrawn_class']?> <?=$row['redrawn_title']?>">
<!--                data-messageview="--><?php //=$row['lastmsg']?><!--"                -->
                <?=$row['redrawn_cell']?>
            </div>
            <div class="proof_vector artdata <?=$row['vectorized_class']?> <?=$row['vectorized_title']?>">
<!--                data-messageview="--><?php //=$row['lastmsg']?><!--"                -->
                <?=$row['vectorized_cell']?>
            </div>
            <div class="proof_proofed artdata <?=$row['proofed_class']?> <?=$row['proofed_title']?>">
<!--                data-messageview="--><?php //=$row['lastmsg']?><!--"                -->
                <?=$row['proofed_cell']?>
            </div>
            <div class="proof_approve artdata <?=$row['approved_class']?> <?=$row['approved_title']?>">
<!--                data-messageview="--><?php //=$row['lastmsg']?><!--"                -->
                <?=$row['approved_cell']?>
            </div>
        </div>
