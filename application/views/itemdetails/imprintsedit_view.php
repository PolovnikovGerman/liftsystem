<?php foreach ($imprint as $row) {?>
    <div class="imprintdatarow">
        <div class="location_name_tab locationedit" data-idx="<?=$row['item_inprint_id']?>">
            <?=($row['item_inprint_location']=='' ? 'Click to add location' : $row['item_inprint_location']) ?>
        </div>
        <div class="location_size_tab"><?= $row['item_inprint_size'] ?></div>
        <div class="location_view_tab">
            <?php if ($row['item_inprint_view']!='') { ?>
                <div class="location_upload edit" data-title="<?= $row['item_inprint_location'] ?>" data-srclink="<?= $row['item_inprint_view'] ?>">
                    View
                </div>
            <?php } ?>
