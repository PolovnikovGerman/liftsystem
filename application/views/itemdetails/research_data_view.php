<table border="0" cellspacing="0" cellpadding="0" align="left" class="researchprices">
    ...
    <?php foreach ($research_price as $row) { ?>
        ...
        <tr align="center">
            <td class="td-text competitorname" data-content="Last Updated Date<br/><?=date('m/d/y H:i:s', $row['other_vendorprice_updated'])?>"><?= $row['vendor_name'] ?></td>
            <?php foreach ($price_types as $pricerow) { ?>
                <td class="td-text2 <?= $row['price_' . $pricerow['type'] . '_class'] ?>" id="priceval<?= $pricerow['type'] ?><?= $counter ?>">
                    <?= (floatval($row['other_vendorprice_price_' . $pricerow['type']]) != 0 ? MoneyOutput($row['other_vendorprice_price_' . $pricerow['type']]) : 'n/a') ?>
                </td>
            <?php } ?>
            <td class="pricing_empty" style="padding-left:6px;">&nbsp;</td>
            <td class="td-text2 <?= $row['price_print_class'] ?>" id="pricevalprint<?= $counter ?>">
                <?= (floatval($row['other_vendorprice_price_print']) != 0 ? MoneyOutput($row['other_vendorprice_price_print']) : 'n/a') ?>
            </td>
            <td class="td-text2 <?= $row['price_setup_class'] ?>" id="pricevalsetup<?= $counter ?>">
                <?= (floatval($row['other_vendorprice_price_setup']) != 0 ? MoneyOutput($row['other_vendorprice_price_setup']) : 'n/a') ?>
            </td>
