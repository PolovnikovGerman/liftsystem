...
<div class="netprofitabledataarea">
    ...
    <?php foreach ($data as $row) { ?>
        <div class="line_table_netprofit <?= ($nrow % 2 == 0 ? 'grey' : 'white' ) ?>" id="nerpr<?= $row['datord'] ?>">
            <div class="tax_quarter <?= $row['tax_quarter_class'] ?>"><?= $row['tax_quarter']?></div>
            <div class="cell_week2 <?=$row['datarowclass']?>" data-profit="<?=$row['profit_id'] ?>"><?= $row['week'] ?></div>
            <div class="cell_sales2 <?=$row['datarowclass']?>" <?= ($row['pcssold'] == 0 ? '' : 'title="# Pcs Sold ' . QTYOutput($row['pcssold']) . '"') ?>><?= $row['sales'] ?></div>
            <div class="cell_revenue2 <?=$row['datarowclass']?>"><?= $row['out_revenue'] ?></div>
            <div class="cell_gross_profit2 <?= $row['profit_class'] ?> <?=$row['datarowclass']?>" data-profitid="/finance/netprofit_orders/?d=<?= $row['profit_id'] ?>"><?= $row['out_profit'] ?></div>
            ...
            <div class="cell_inbut2 <?=$row['notesclass']?> <?=$row['datarowclass']?>">
                <div class="imbox <?=($row['shownote']==1 ? 'shownote' : '')?>" data-netnote="/finance/netprofit_weeknoteview/?d=<?= $row['profit_id'] ?>">
