...
//    function get_batches_limits() {
//        $this->db->select('min(batch_due) as min_date, max(batch_due) as max_date');
//        $this->db->from('v_order_calendbatches');
//        $res=$this->db->get()->row_array();
//        return $res;
//    }
//
//    function get_calendar_view($options) {
//        $ci=&get_instance();
//        $this->db->select('*');
//        $this->db->from('v_order_calendbatches');
//        $this->db->order_by('batch_due');
...
//            $row['out_vmd']=($sum==0 ? '&nbsp;' : ($sum>0 ? '$'.number_format($sum,2,'.','') : '($'.number_format(abs($sum),2,'.','').')').Batches_model::VISA);
...
//            $row['out_amex']=($sum==0 ? '&nbsp;' : ($sum>0 ? '$'.number_format($sum,2,'.','') : '($'.number_format(abs($sum),2,'.','').')').Batches_model::AMEX);
...
//            $row['out_other']=($sum==0 ? '&nbsp;' : ($sum>0 ? '$'.number_format($sum,2,'.','') : '($'.number_format(abs($sum),2,'.','').')').Batches_model::OTHERPAY);
...
//            $row['out_term']=($sum==0 ? '&nbsp;' : ($sum>0 ? '$'.number_format($sum,2,'.','') : '($'.number_format(abs($sum),2,'.','').')').Batches_model::TERMS);
//            // $row['out_term']=(floatval($row['deb_term'])==0 ? '&nbsp;' : '$'.number_format($row['deb_term'],2,'.','').Batches_model::TERMS);
//            $row['out_writeoffclass']=(floatval($row['deb_writeoff'])<0 ? 'negative' : '');
//            $sum=floatval($row['deb_writeoff']);
//            $row['out_writeoff']=($sum==0 ? '&nbsp;' : ($sum>0 ? '$'.number_format($sum,2,'.','') : '($'.number_format(abs($sum),2,'.','').')').Batches_model::WRITE_OFF);
//            $dateres[]=$row;
//        }
//        $out=array();
...
//        $weeks=$ci->func->dates_diff($date_min, $date_max, 'W');
//        if ($weeks<5) {
//            $weeks=5;
//        } else {
//            $weeks=$weeks+2;
...
//    }
//
//    function get_calend_totals($options) {
//        $empty='---';
//        $this->db->select('sum(total_orders) as sumord, sum(inv_vmd) as sum_inv_vmd, sum(inv_amex) as sum_inv_amex');
//        $this->db->select('sum(inv_other) as sum_inv_other, sum(inv_term) as sum_inv_term');
//        $this->db->select('sum(deb_vmd) as sum_deb_vmd, sum(deb_amex) as sum_deb_amex, sum(deb_other) as sum_deb_other, sum(deb_term) as sum_deb_term');
//        $this->db->select('sum(inv_writeoff) as sum_inv_writeoff, sum(deb_writeoff) as sum_deb_writeoff');
//        $this->db->from('v_order_batches');
//        $res=$this->db->get()->row_array();
//        /* Total past */
...
//    }
//
//    function get_batchdetails($options) {
//        set_time_limit(0);
...
//                $this->db->where('deb_total != ',0);
//            } elseif($options['received']==1) {
//                $this->db->where('receiv_total != ',0);
//            }
//        } else {
...
//        }
//        $res=$this->db->order_by('batch_date','desc');
//        $res=$this->db->get()->result_array();
//        $out=array();
//        $numpp=1;
//        foreach ($res as $row) {
...
//                $lrow['batchnote']='<img src="/img/empty_square.png" alt="empty note"/>';
//                $lrow['batchnote_class']='';
//                $lrow['batchnote_title']='';
//                if ($lrow['batch_note']!='') {
//                    $lrow['batchnote']='<img src="/img/lightblue_square.png" alt="Batch note"/>';
...
//    }
    ...
    function get_batchdetails_date($options) {
        $empty='---';
        $this->db->select('*');
        $this->db->from('v_order_batches');
        $this->db->where('batch_date',$options['batch_date']);
        $res=$this->db->get()->row_array();
...
//    }
//
//    function batchreceived($batch_id,$receiv) {
...
//
//    /* batches by due date */
//    function get_batchcalen_date($options) {
//        $this->db->select('*');
//        $this->db->from('v_order_calendbatches');
//        $this->db->where('batch_due',$options['batch_due']);
...
//                $duedate=$ci->func->getVMDDueDate($options['batch_date'],$paymethod);
//            } elseif($batch_amex!=0) {
//                $batch_amount=$batch_amex;
//                $duedate=$ci->func->getAmexDueDate($options['batch_date'],$paymethod);
...
//    }
//
//    function batch_freebatchadd($newrec) {
