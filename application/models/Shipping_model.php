...
//        return $res;
    ...
    public function count_quoteshiprates_new($items, $quote, $deliv_date, $brand, $default_ship_method='')
    {
        ...
        if (!empty($quote['shipping_zip'])) {
            ...
            $rates = $this->calculate_shipcostdefault($quote['shipping_country'], $quote['brand'], $totalqty, $deliv_date);
            $codearray = array_keys($rates);
            $outrate = [];
            if ($default_ship_method == '') {
                if (isset($rates['GND'])) {
                    $rates['GND']['current'] = 1;
                } elseif (isset($rates['UPSStandard'])) {
                    $rates['UPSStandard']['current'] = 1;
                } elseif (isset ($rates['UPSExpedited'])) {
                    $rates['UPSExpedited']['current'] = 1;
                } elseif (isset($rates['UPSSaver'])) {
                    $rates['UPSSaver']['current'] = 1;
                }
            } else {
                foreach ($codearray as $coderow) {
                    if ($rates[$coderow]['ServiceName'] == $default_ship_method) {
                        $rates[$coderow]['current'] = 1;
                    }
                }
            }
